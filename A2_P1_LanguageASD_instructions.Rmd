...
#more models
p0 <- lmer(MOT_MLU ~ VISIT + (1 | ID), cleanData, REML = F )
p1 <- update(p0, .~. + Diagnosis)
p2 <- update(p1, .~. + VISIT:Diagnosis)
p3 <- lmer(MOT_MLU ~ VISIT + (1 + VISIT | ID), cleanData, REML = F)
p4 <- update(p3, .~. + Diagnosis)
p5 <- update(p4, .~. + VISIT:Diagnosis) #fails to converge

anova(p0, p3, p1, p4, p2)
summary(p4)

# Growth curve based on the best model, p4
p_growth2 <- lmer(MOT_MLU ~ VISIT + Diagnosis + I(VISIT^2) + (1 + VISIT | ID), cleanData, REML = F)
p_growth3 <- lmer(MOT_MLU ~ VISIT + Diagnosis + I(VISIT^3) + (1 + VISIT | ID), cleanData, REML = F)
anova(p4, p_growth2, p_growth3)

summary(p_growth3)

#anova (p_growth3, test = 'F')
