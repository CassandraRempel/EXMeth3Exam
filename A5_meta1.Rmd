...
ranktest(m1)
funnel(m1)
sum(df$SAMPLE_SIZE_HC + df$SAMPLE_SIZE_SZ, na.rm = T)
sum(df$SAMPLE_SIZE_HC, na.rm = T)
sum(df$SAMPLE_SIZE_SZ, na.rm = T)
```
...
```{r}
m3 <- rma (yi, vi, data = df_pitch_es, slab = StudyID)
summary(m3)
forest(m3)
inf3 <- influence(m3)
plot(inf3)
