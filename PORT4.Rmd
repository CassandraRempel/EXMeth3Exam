...
long_data <- long_data %>%
  mutate (participant_num = as.numeric(str_extract(participant_id, ("\\d"))),
          participant_id = as.numeric(paste0(group_id, str_extract(participant_id, ("\\d")))))
...
hr_self <- pivot_longer(wide_surrogate_data, c(HR1_scaled,HR2_scaled), names_to = "participant", values_to = "HR_self") 
hr_other <- pivot_longer(wide_surrogate_data, c(HR2_scaled,HR1_scaled), values_to = "HR_other") 
...
resp_self <- pivot_longer(wide_surrogate_data, c(Resp1_scaled, Resp2_scaled), values_to = "Resp_self")
resp_other <- pivot_longer(wide_surrogate_data, c(Resp2_scaled, Resp1_scaled), values_to = "Resp_other")
...
hr_change <- pivot_longer(wide_surrogate_data, c(HR1_change, HR2_change), values_to = "HR_self_change")
hr_change_other <- pivot_longer(wide_surrogate_data, c(HR2_change, HR1_change), values_to = "HR_other_change")

#adding to df
df_surrogate <- cbind(df_surrogate, 
                 HR_self_change = hr_change$HR_self_change, 
                 HR_other_change = hr_change_other$HR_other_change)

#now we do it with resp change self and other
resp_change <- pivot_longer(wide_surrogate_data, c(Resp1_change, Resp2_change), values_to = "Resp_self_change")
resp_change_other <- pivot_longer(wide_surrogate_data, c(Resp2_change, Resp1_change), values_to = "Resp_other_change")

#adding to df
df_surrogate <- cbind(df_surrogate, 
                 Resp_self_change = resp_change$Resp_self_change, 
                 Resp_other_change = resp_change_other$Resp_other_change)

#now it's time for hr lag self and hr lag other 
hr_lag <- pivot_longer(wide_surrogate_data, c(HR1_lag, HR2_lag), values_to = "HR_lag_self")
hr_lag_other <- pivot_longer(wide_surrogate_data, c(HR2_lag, HR1_lag), values_to = "HR_lag_other")
...
resp_lag <- pivot_longer(wide_surrogate_data, c(Resp1_lag, Resp2_lag), values_to = "Resp_lag_self")
resp_lag_other <- pivot_longer(wide_surrogate_data, c(Resp2_lag, Resp1_lag), values_to = "Resp_lag_other")
...
df_surrogate <- df_surrogate %>% 
  mutate(participant_num = as.numeric(str_extract(participant, ("\\d"))),
         participant_id = as.numeric(ifelse(participant_num == 1, 
                                 paste0(str_extract(partner1_id, ("\\d+")), participant_num), 
                                 paste0(str_extract(partner2_id, ("\\d+")), participant_num))))


#adding column to specify whether it is surrogate or real 
df_surrogate$type = "surrogate"
df_surrogate <- df_surrogate %>% 
  mutate(group_id = as.numeric(paste0(str_extract(df_surrogate$partner1_id, "\\d+"), str_extract(df_surrogate$partner2_id, "\\d+"))),
         condition = str_extract(df_surrogate$partner1_id, "[A-z]+"))


#write to csv
write_csv(df_surrogate, "surrogate_master.csv")
```
...
df_long_subset <- long_data %>% 
  select(starts_with("HR"), starts_with("Resp"), group_id, participant_id, participant_num, type, condition)

df_shuffle_subset <- shuffle_data %>% 
  select(starts_with("HR"), starts_with("Resp"), group_id, participant_id, participant_num, type, condition)

df_surrogate_subset <- df_surrogate %>% 
  select(starts_with("HR"), starts_with("Resp"), group_id, participant_id, participant_num, type, condition)


#making a master df
shuffle_long_df <- rbind(df_long_subset, df_shuffle_subset)
surrogate_long_df <- rbind(df_long_subset, df_surrogate_subset)
full_df <- rbind(shuffle_long_df, df_surrogate_subset)


#Make variables into factors
shuffle_long_df <- shuffle_long_df %>% 
  mutate(group_id = as.factor(group_id),
         participant_id = as.factor(participant_id),
         participant_num = as.factor(participant_num),
         type = as.factor(type),
         condition = as.factor(condition))

#Make variables into factors
surrogate_long_df <- surrogate_long_df %>% 
  mutate(group_id = as.factor(group_id),
         participant_id = as.factor(participant_id),
         participant_num = as.factor(participant_num),
         type = as.factor(type),
...
full_df <- within(full_df, type <- relevel(type, ref = 'real'))
...
```{r}
# Create models as in chunks above, but adding an interaction with the Real vs. Surrogate variable
#Models without conditions

#Compare to surrogate data
HR_model_comparison_surrogate <- lmer(HR_self_change ~ 0 + (HR_self + HR_other) * type + (1 | participant_id) + (1 | group_id), surrogate_long_df, REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(HR_model_comparison_surrogate)

resp_model_comparison_surrogate <- lmer(Resp_self_change ~ 0 + (Resp_self + Resp_other) * type + (1 | participant_id) + (1 | group_id),  surrogate_long_df, REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(resp_model_comparison_surrogate)


#compare to shuffled data
HR_model_comparison_shuffled <- lmer(HR_self_change ~ 0 + (HR_self + HR_other) * type + (1 | participant_id) + (1 | group_id), shuffle_long_df, REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(HR_model_comparison_shuffled)

resp_model_comparison_shuffled <- lmer(Resp_self_change ~ 0 + (Resp_self + Resp_other) * type + (1 | participant_id) + (1 | group_id),  shuffle_long_df, REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(resp_model_comparison_shuffled)




#Models with conditions
HR_model_conditions <- lmer(HR_self_change ~ 0 + (HR_self + HR_other) * condition * type + (condition | participant_id) + (condition | group_id), full_df, REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(HR_model_conditions)

resp_model_conditions <- lmer(Resp_self_change ~ 0 + (Resp_self + Resp_other) * condition * type + (condition | participant_id) + (condition | group_id),  full_df, REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
summary(resp_model_conditions)
```
