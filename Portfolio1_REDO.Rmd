...
```{r}
#making visit homogenous by extracting the digit and making all the coloumns numerical. 
lu_train$VISIT <- str_extract(lu_train$VISIT, "\\d")
token_train$VISIT <- str_extract(token_train$VISIT, "\\d")

#recode 1 to visit1. to homogeneize the data, this makes too much mess -bad
...
s_demo <- select(demo_train, ID, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
s_token <- select(token_train, ID, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
s_lu <- select(lu_train, ID, VISIT, MOT_MLU, CHI_MLU)
...
```{r}
#HERE IS WHERE THE VISIT is lost
...
# merge the dfs 
#different way to solve, not as good
