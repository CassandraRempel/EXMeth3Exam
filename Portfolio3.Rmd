...
summary (master_data$study_nr)

one <- subset(master_data, study_nr =="1")
two <- subset(master_data, study_nr =="2")
three <- subset(master_data, study_nr =="3")
four <- subset(master_data, study_nr =="4")
...
#how to figure out na's and which ones have na's, how many are observed with sum
...
#because we normalized the variables our estimates have become standard deviations.
...
ggplot(new_plot_data, aes(x=who, y=effect, colour=who)) +
    ...
    theme_minimal() +
    labs(title = "Comparison of Hedge's g for meta-analysis", x=NULL, y = "Hedge's g") +
